...
Useful table of abbreviations for countries :
...
#| echo: false
filtered_data <- merged_data %>%
  filter(year == 2022, geo %in% c("FR", "DE", "BG", "EU27_2020"))
...
filtered_data2 <- merged_data2 %>%
  filter(geo %in% c("FR", "DE", "BG"))  # Replace with desired country codes
...
We observe notable differences in patent applications between the two
groups on the map. Specifically, Western European countries tend to have
higher patent application rates compared to their counterparts in
...
We observe that the "Other" group exhibits significantly higher patent averages and variability, indicating that it includes countries or entities with substantially greater patent output. In contrast, CEE countries demonstrate much lower patent activity and variability compared to both WE and the "Other" group.

The R-squared value for the Western group is lower than that of the Eastern group, suggesting that the variation in patent applications among Eastern countries is better explained by the variables in our model (sector and educational attainment). However, it is important to note that there are 495 more observations in the Central and Eastern group compared to the Western group. This disparity in sample size impacts the R-squared values; larger groups tend to produce more stable and robust R-squared estimates, while smaller groups may result in less reliable values.

To draw definitive conclusions, additional computations are required. For this reason, we use the adjusted R-squared, as it is less sensitive to differences in sample size. This metric provides a more accurate assessment of how well the model fits the data, especially when comparing groups with unequal sample sizes.
...
cat("RSS Pooled:", rss_pooled, "\n")
cat("RSS Western:", rss_western, "\n")
cat("RSS Eastern:", rss_eastern, "\n")
```
The residual some of square for western countries is: 9300486501 and for central and eastern european countries is: 63183986.
...
The Chow test was conducted to assess potential structural breaks between the "Western" and "Eastern" country groups. However, the results indicate potential issues:
The calculated F-statistic is negative (F=−3365.676).This suggests a problem with the test setup or the underlying data.
The p-value was not calculable (p=NaN), further confirming issues in the analysis.
There's two possible reason for this issue:
  1) Scaling Issues: The pooled regression includes significantly more observations than the group-specific regressions, leading to discrepancies in residual sum of squares (RSS) comparisons.
  2) Data Imbalance: Unequal group sizes may have caused instability in the calculation of the Chow test statistic.
That's the reason why The Chow test could not provide a reliable conclusion regarding structural breaks due to these limitations. Further analysis with more balanced or scaled data would be required for a robust test.
...
To examine whether the relationship between educational attainment and patents differs across country groups, we compared two regression models using a likelihood ratio test.

Model 1 included educational attainment, sector, and year as predictors of patents.
Model 2 expanded on Model 1 by incorporating interaction terms between educational attainment and country group, allowing the effect of educational attainment to vary across country groups.
The likelihood ratio test indicated that Model 2 provides a significantly better fit to the data (F-statistic = 148.26, p-value < 2.2e-16). The inclusion of the interaction terms led to a substantial reduction in the residual sum of squares (RSS), suggesting that the relationship between educational attainment and patents differs across country groups.
