...
# Remplacez "data" par le nom réel de votre dataframe

# Création du graphique
ggplot(filtered_data, aes(x = year, y = patents, fill = educational_attainment)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ geo, scales = "free") +
  scale_fill_viridis_d(name = "Educational Attainment") +
  theme_minimal() +
  labs(
    x = "Year",
    y = "Number of Patents",
    title = "Number of Patents by Educational Attainment for Each Country (by Year)"
  ) +
  theme(
...
# Analysis on education in coutries
...
education_analysis |>
  ...
  scale_fill_viridis_d(name = "Education Level") +  # Palette de couleurs
  theme_minimal() +  
  labs(
...
```{r}
filtered_data <- merged_data %>%
  filter(geo %in% c("FR", "DE", "BG"),  # Selected countries
    year %in% c(2012, 2015, 2018, 2021))# Show only these years

plot_education <- ggplot(filtered_data, aes(x = year, y = education))
```
