...
```{r}
panel_model <- plm(patents.x ~ educational_attainment + sector,
                  data = combined_data,
...
```{r}
 # Create a binary variable for Western countries (1 for Western, 0 for non-Western)
vet <- vet %>%
  mutate(is_western_binary = ifelse(country_group == "WE", 1, 0))
```
...
# Step 3: Handle missing data (check for missing values in necessary columns)
vet <- vet %>%
  filter(!is.na(patents.x) & !is.na(is_western_binary))
```

```{r}
# Step 4: Run RDD (Regression Discontinuity Design) using the existing binary indicator
m <- rdrobust(vet$patents.x, vet$is_western_binary, c = 0)

# Step 5: Summarize and plot the RDD result
summary(m)
rdplot(vet$patents.x, vet$is_western_binary)
```


```{r}
# Fit a linear model with interaction terms for country group and other predictors
mw <- lm(patents.x ~ educational_attainment * country_group +
...
binned <- vet %>%
  ...
  summarize(avg_patents = mean(patents.x, na.rm = TRUE))  # Ensure to handle missing values
```
