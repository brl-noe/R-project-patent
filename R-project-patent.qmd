...
=======
>>>>>>> eb3095dfa91cd4f3a627655b0f26d6afe442069d
...
Then we label the education attainements :
```{r}
...
<<<<<<< HEAD
education_analysis |>
  filter(geo == "BG") |>  # Changer "FR" par "BG" pour la Bulgarie
  pivot_longer(
    cols = starts_with("19") | starts_with("20"), 
    names_to = "Years", 
    values_to = "Value"
  ...
  mutate(
    Years = as.numeric(Years),           # Transformer les valeurs de Years en format numérique 
    Value = as.numeric(Value)            # Même chose pour Value
  ) |>
  group_by(Years) |>
  mutate(Proportion = Value / sum(Value, na.rm = TRUE)) |>  # Proportion par niveau d'éducation par année
  ggplot(aes(x = Years, y = Proportion, fill = isced11)) +
  geom_bar(stat = "identity", position = "stack") +  
  scale_fill_viridis_d(name = "Education Level") +  
  theme_minimal() +  
...
more likely that this will have patent than an other. 
=======
#| message: false
#| echo: false
#| warning: false
...
plot_education_patents2 <- ggplot(filtered_data, 
    ...
    geom_bar(stat = "identity", position = "stack", width = 0.7, color = "black") +  # Adjust the borders of the sitcks
    geom_text(aes(label = paste0(round(proportion, 1), "%")), 
              position = position_stack(vjust = 0.5), size = 3) +  
    scale_fill_viridis_d(name = "Education Level", option = "C") +  
    theme_minimal() +
    labs(x = "Country",
         y = "Number of Patents",
...
## C-Patent and key indicator application
...
sector_analysis <- key_indicator2 |>
  filter(sector != "Total economy") |> # For 
...
# 2. Create the plot
plot_key_patents <- ggplot(
  filtered_data2, 
  aes(
    x = year, 
    y = patents, 
    color = sector,  # Different color for each sector
    linetype = sector,  # Different line type for each sector
    group = sector
  )
) +
  # Add lines
...
```
As we can see on the graph, our data is not as clean as wished however we still
can see the role of each sector for patent. In western Europe countries we 
notice the importance of governments and Households Non-profit institutions 
serving households (NPISH). Since, France and Germany have more patent over the
years than Bulgaria, it seems that the more patent governments and Households 
and NPISHs have a role in the economy, the more patent you get. 
...
filtered_data3 <- merged_data2 %>%
    group_by(geo) %>%
    mutate(proportion = patents / sum(patents) * 100)  # add the proportion in percentage

plot_key_patents2 <- ggplot(filtered_data3, 
         aes(x = geo, 
             y = patents, 
             fill = sector)) +
    geom_bar(stat = "identity", position = "stack", width = 0.7) +  # Adjust the borders of the sitcks
    geom_text(aes(label = paste0(round(proportion, 1), "%")), 
              position = position_stack(vjust = 0.5), size = 3) +  
    scale_fill_viridis_d(name = "Sector", option = "C") +  
    theme_minimal() +
    labs(x = "Country",
         y = "Number of Patents",
...
The calculated F-statistic is negative (F=−3365.676), which is not meaningful in this context. This suggests a problem with the test setup or the underlying data.
The p-value is not calculable (p=NaN), further confirming issues in the analysis.
Possible Causes:
  Scaling Issues: The pooled regression includes significantly more observations than the group-specific regressions, leading to discrepancies in residual sum of squares (RSS) comparisons.
  Data Imbalance: Unequal group sizes may have caused instability in the calculation of the Chow test statistic.
The Chow test could not provide a reliable conclusion regarding structural breaks due to these limitations. Further analysis with more balanced or scaled data would be required for a robust test.
