...
#| message: false
#| echo: false
install.packages("conflicted", "here", "vroom","dplyr","ggplot2","tidyr","stringr", "tidyverse","plm","AER", "sandwich")
library(conflicted)
library(here)
library(vroom)
library(dplyr)
...
### Education and patent application

```{r}
# Long data conversion
long_data <- combined_data %>%
  pivot_longer(cols = starts_with("20") & ends_with(".x"),  
               names_to = "year", 
               values_to = "value") %>%
  mutate(year = as.numeric(year))
```
...
```{r}
# Sample 10% of observations per year
sampled_data <- long_data %>%
  group_by(year) %>%                    # Group by year
  sample_frac(0.1) %>%                   # Sample 10% of rows per year
  ungroup()                             # Remove the grouping

# Generate the plot with the sampled data
ggplot(sampled_data, aes(x = educational_attainment, y = value, color = factor(year))) + 
  geom_point(alpha = 0.1) + 
  geom_smooth(method = "lm", formula = y ~ x, aes(group = year))
```
...
Similarly as before, if we only consider the 27 countries of the euro zone,
